...
import { 
    ...
    ChevronDown, ChevronRight, ArrowRight, Activity, Laptop, LayoutGrid, Focus, Check, Minus, Workflow 
} from 'lucide-react';
                    ...
                    <div className="space-y-2">
                        {immediateEdges.map((item, idx) => (
                            <div 
                                ...
                                className="flex items-center gap-2 p-2 rounded bg-slate-800/30 border border-slate-800 hover:bg-slate-800 hover:border-slate-600 transition-all cursor-pointer group"
        ...
        <CollapsibleSection title="Focus Cluster" icon={<Focus size={14} />} defaultOpen={true}>
            <div className="space-y-1">
                <button
                    ...
                    className={`w-full text-left px-2 py-1.5 rounded text-xs flex items-center justify-between transition-colors ${activeModule === null ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200'}`}
                >
                    <span>All Modules</span>
                    {activeModule === null && <div className="w-1.5 h-1.5 rounded-full bg-white"></div>}
                </button>
                {Object.entries(moduleCounts).sort((a: [string, number], b: [string, number]) => b[1] - a[1]).map(([moduleName, count]) => (
                    <button
                        ...
                        className={`w-full text-left px-2 py-1.5 rounded text-xs flex items-center justify-between transition-colors ${activeModule === moduleName ? 'bg-emerald-600/20 text-emerald-400 border border-emerald-500/30' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200'}`}
                    >
                        <span className="truncate pr-2">{moduleName}</span>
                        <span className="text-[10px] opacity-60 bg-slate-900 px-1 rounded">{count}</span>
                    </button>
                ))}
            </div>
        </CollapsibleSection>
